...
def run(BASE_DIR):
    ...
    # Gráficas en toneladas
    st.subheader("Distribución Peso Neto Exportado (t)")
    fig1 = px.histogram(df, x=df["peso neto exportado"] / 1000, nbins=30, color_discrete_sequence=[COLOR_LOGO])
    st.plotly_chart(fig1, use_container_width=True)

    st.subheader("Distribución Peso Neto Importado (t)")
    fig2 = px.histogram(df, x=df["peso neto importado"] / 1000, nbins=30, color_discrete_sequence=[COLOR_LOGO])
    st.plotly_chart(fig2, use_container_width=True)

    st.subheader("Distribución Peso Total (t)")
    fig3 = px.histogram(df, x=df["peso total (t)"], nbins=30, color_discrete_sequence=[COLOR_LOGO])
