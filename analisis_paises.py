...
def run(BASE_DIR):
    ...
    fig_exp = px.bar(paises, x=paises["destino"], y=paises["peso neto exportado"] / 1000, color_discrete_sequence=[COLOR_LOGO])
    st.plotly_chart(fig_exp, use_container_width=True)

    st.subheader("Importaciones por País (t)")
    fig_imp = px.bar(paises, x=paises["destino"], y=paises["peso neto importado"] / 1000, color_discrete_sequence=[COLOR_LOGO])
    st.plotly_chart(fig_imp, use_container_width=True)

    st.subheader("Peso Total por País (t)")
    fig_total = px.bar(paises, x=paises["destino"], y=paises["peso total (t)"], color_discrete_sequence=[COLOR_LOGO])
    st.plotly_chart(fig_total, use_container_width=True)

    st.subheader("Mapa de Exportaciones (t)")
    fig_map = px.choropleth(
        paises,
        locations=paises["destino"],
        locationmode="country names",
        color=paises["peso neto exportado"] / 1000,
