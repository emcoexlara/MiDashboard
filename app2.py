...
with col1:
    st.markdown(f"<div style='background-color:{COLOR_CUADRO}; padding:15px; border-radius:10px; text-align:center;'>"
                f"<h3>Operaciones</h3>"
                f"<p style='font-size:24px'>{len(df_filtrado)}</p></div>", unsafe_allow_html=True)

with col2:
    st.markdown(f"<div style='background-color:{COLOR_CUADRO}; padding:15px; border-radius:10px; text-align:center;'>"
                f"<h3>Exportado (t)</h3>"
                f"<p style='font-size:24px'>{df_filtrado['PESO NETO EXPORTADO'].sum():,.2f}</p></div>", unsafe_allow_html=True)

with col3:
    st.markdown(f"<div style='background-color:{COLOR_CUADRO}; padding:15px; border-radius:10px; text-align:center;'>"
                f"<h3>Importado (t)</h3>"
                f"<p style='font-size:24px'>{df_filtrado['PESO NETO IMPORTADO'].sum():,.2f}</p></div>", unsafe_allow_html=True)
    with col4:
    st.markdown(f"<div style='background-color:{COLOR_CUADRO}; padding:15px; border-radius:10px; text-align:center;'>"
                f"<h3>Total (t)</h3>"
                f"<p style='font-size:24px'>{df_filtrado['PESO TOTAL'].sum():,.2f}</p></div>", unsafe_allow_html=True)
